...
export default function Formulario() {
      ...
      <div className="min-h-screen bg-gradient-to-b from-primary-50 to-white py-16">
        <div className="max-w-3xl mx-auto px-4">
          <div className="text-center mb-10">
            ...
            <h1 className="text-3xl md:text-4xl font-bold text-gray-800 mb-4">Avaliação Psicológica Inicial</h1>
            <p className="text-gray-600 text-lg max-w-xl mx-auto">
              Para oferecer um atendimento personalizado, precisamos conhecer melhor o perfil do paciente. Selecione a faixa etária abaixo para começar.
            </p>
          </div>

          <div className="bg-white rounded-2xl shadow-lg p-8 mb-8">
            <h2 className="text-xl font-semibold text-gray-800 text-center mb-6">Quem será avaliado(a)?</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {faixasEtarias.map((faixa) => (
                <button
                  key={faixa.value}
                  onClick={() => setFaixaEtaria(faixa.value as FaixaEtaria)}
                  className="bg-gradient-to-r from-primary-50 to-white border-2 border-primary-200 rounded-xl p-6 text-left hover:border-primary-500 hover:from-primary-50 hover:to-primary-50 transition-all hover:shadow-md group"
                >
                  <span className="text-lg font-semibold text-gray-800 group-hover:text-primary-600 transition-colors">{faixa.label}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="bg-accent-50 border border-accent-200 rounded-xl p-6 text-center">
            <p className="text-accent-700 font-medium">
              Avaliação sem compromisso. Suas respostas são sigilosas e protegidas pela LGPD.
            </p>
          </div>
        </div>
      </div>
  ...
  if (isSubmitted) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-accent-50 to-white py-20">
        <div className="max-w-2xl mx-auto px-4 text-center">
          ...
          <p className="text-gray-600 mb-8 text-lg">
            Obrigado por preencher! Envie suas respostas diretamente para a psicóloga via WhatsApp e agende sua primeira sessão.
          </p>
          <a
            href={`https://wa.me/5568999035300?text=${whatsappMsg}`}
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-3 bg-green-500 text-white px-10 py-4 rounded-xl font-bold hover:bg-green-600 transition-colors text-lg shadow-lg hover:shadow-xl"
          >
            <Send className="w-6 h-6" />
            Enviar via WhatsApp
          </a>
          <p className="text-sm text-gray-500 mt-6">
            Após o envio, a psicóloga entrará em contato para agendar a primeira sessão.
          </p>
        </div>
      </div>
    )
